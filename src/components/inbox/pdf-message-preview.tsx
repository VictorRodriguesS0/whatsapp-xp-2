...
export function PdfMessagePreview({
  ...
  if (state === "failed") {
    return (
      <button
        aria-label={label}
        className="inline-flex min-h-11 max-w-full items-center gap-2 rounded-md border border-[var(--border)] px-3 text-left font-semibold text-[var(--accent)] outline-none hover:bg-white/50 focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
        onClick={(event) => {
  ...
  return (
    <button
      aria-label={label}
      className="block min-h-11 w-[min(16rem,70vw)] max-w-full overflow-hidden rounded-md border border-[var(--border)] bg-[var(--canvas)] text-left outline-none hover:border-[color-mix(in_srgb,var(--accent)_45%,var(--border))] focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
      onClick={(event) => {
        event.currentTarget.focus();
        onOpen();
      }}
    ...
    >
      ...
      <span className="flex min-h-11 items-center gap-2 border-t border-[var(--border)] bg-white/55 px-3">
        <FileText aria-hidden="true" className="size-4 shrink-0 text-rose-600" />
