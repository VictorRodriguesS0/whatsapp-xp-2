...
export function MessageComposer({
  ...
  return (
    <form
      className="message-composer relative border-t border-[var(--border)]"
    ...
    >
      ...
      {recorder.phase === "recording" ? (
        <div className="message-composer__state flex min-h-11 flex-wrap items-center gap-2">
          ...
          <Button aria-label="Parar gravação" className="min-h-11 min-w-11" disabled={unavailable} onClick={stopRecording} size="icon">
      ...
      {recorder.phase !== "requesting" && recorder.phase !== "recording" && recorder.phase !== "preview" ? (
        <>
          ...
          <div className="message-composer__row flex min-w-0 items-end gap-2">
            <input
              ref={fileInputRef}
            ...
            <textarea
              ...
              className="message-composer__field min-h-11 min-w-0 max-h-36 flex-1 resize-y rounded-md border border-[var(--border)] bg-[var(--surface-elevated)] px-3 py-2.5 text-base text-[var(--text)] outline-none placeholder:text-[var(--muted)] focus-visible:border-[var(--accent)] focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--accent)_25%,transparent)]"
              disabled={disabled}
              id="message-body"
            ...
            />
            {!body.trim() && !file ? (
              <Button asChild className="min-h-11 min-w-11" size="icon">
                <button
              ...
              <Button aria-label="Enviar mensagem" className="min-h-11 min-w-11" disabled={disabled || (!body.trim() && !file)} size="icon" type="submit"><Send aria-hidden="true" className="size-4" /></Button>
            )}
          </div>
        </>
      ) : null}
