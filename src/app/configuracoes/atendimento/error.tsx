...
export default function ClassificationError({ reset }: { error: Error & { digest?: string }; reset(): void }) {
  return (
    <main className="flex min-h-dvh items-center justify-center bg-[var(--canvas)] px-5 py-10">
      <section aria-labelledby="classification-error-heading" className="w-full max-w-lg border-t-4 border-[var(--danger)] bg-[var(--panel)] px-7 py-8">
        <p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--danger)]">Configurações</p>
        <h1 className="mt-2 text-2xl font-bold" id="classification-error-heading">Não foi possível carregar as classificações</h1>
