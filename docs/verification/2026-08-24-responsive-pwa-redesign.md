...
- Manual production acceptance closed the browser-automation gap: after being instructed to install the public HTTPS app in Chrome or Edge, open the installed shortcut and confirm the standalone app window/online-session behavior, the user explicitly replied `INSTALOU E ABRIU`. No credential, private content or screenshot was requested or retained for this confirmation.
